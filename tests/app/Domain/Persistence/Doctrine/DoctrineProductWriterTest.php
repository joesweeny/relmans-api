...
class DoctrineProductWriterTest extends TestCase
{
    ...
    public function test_updateProduct_updates_the_status_column_for_a_product_record()
    {
        ...
        $query = (new ProductWriterQuery())->setStatus(ProductStatus::IN_STOCK());
    ...
    public function test_updateProduct_updates_the_featured_column_for_a_product_record()
    {
        $productId = Uuid::uuid4();
        ...
        $query = (new ProductWriterQuery())->setIsFeatured(false);

        $this->writer->updateProduct($productId, $query);

        $row = $this->fetchRecord($productId, 'product');

        $this->assertFalse($row->featured);
        $this->assertEquals(1738497600, $row->updated_at);
    }

    public function test_updateProduct_throws_a_NotFoundException_if_product_does_not_exist()
    {
        $productId = Uuid::uuid4();

        $this->expectException(NotFoundException::class);
        $this->expectExceptionMessage("Product {$productId} does not exist");
        $this->writer->updateProduct($productId, new ProductWriterQuery());
    }
